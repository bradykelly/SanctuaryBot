...
class Hub(commands.Cog):
    ...
    async def on_guild_join(self, guild):
        await self.bot.db.execute("INSERT OR IGNORE INTO guild_config (guild_id) VALUES (?)", guild.id)
    ...
    async def on_guild_remove(self, guild):
        await self.bot.db.execute("DELETE FROM guild_config WHERE guild_id = ?", guild.id)
